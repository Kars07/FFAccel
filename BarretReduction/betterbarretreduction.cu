...
//Data types for finite field arithmetic
...
__device__ __forceinline__ fp_t better_barrett_reduction(fp_wide_t x) {
    
    const int m = 32; //bit length of prime p(assuming 32 bit length)
    
    fp_wide_t c = x >> m - 2;
    fp_wide_t quot =  c * MU >> m + 3;
...
__device__ __forceinline__ fp_t fp_mul_barrett(fp_t a, fp_t b) {
    ...
    return better_barrett_reduction(prod);
}
...
__device__ fp_t fp_pow_barrett(fp_t base, fp_t exp) {
    fp_t result = 1;
    base = better_barrett_reduction(base); // Ensure base is within the field
...
int main() {
    ...
    printf("Better Barrett Addition test: %u + %u = %u (mod %u)\n", a[0], b[0], result[0], p);
    
    // Test multiplication
    ff.mul_arrays(a, b, result, n);
    printf("Better Barrett Multiplication test: %u * %u = %u (mod %u)\n", a[0], b[0], result[0], p);
    
    // Test inverse (on smaller array for performance)
    const int small_n = 1000;
    ff.inv_arrays(a, result, small_n);
    printf("Better Barrett Inverse test: inv(%u) = %u (mod %u)\n", a[1], result[1], p);
